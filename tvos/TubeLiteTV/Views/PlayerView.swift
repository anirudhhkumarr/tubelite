...
public struct PlayerView: View {
    ...
    private func teardownPlayer() {
        if let token = telemetryTimeObserverToken, let activePlayer = player {
            activePlayer.removeTimeObserver(token)
            telemetryTimeObserverToken = nil
        }
        let finalSec = player.map { CMTimeGetSeconds($0.currentTime()) } ?? 0.0
    ...
    /// Activates tvOS movie playback audio session asynchronously for multichannel pass-through and spatial audio upmixing.
