...
import { createTelemetrySession } from '../playback/telemetryManager.js';
...
export default function PlayerModal({
  ...
  // Initialize watch telemetry session for sync with YouTube account
  useEffect(() => {
    if (!video?.id) return;

    const sessionInstance = createTelemetrySession({
      videoId: video.id,
      getAccessToken: () => store.getValidAccessToken(),
      workerUrl: session?.workerUrl
    });
    telemetryRef.current = sessionInstance;
    sessionInstance.init();

    return () => {
      sessionInstance?.dispose(lastCurrentTimeRef.current);
      telemetryRef.current = null;
    };
  }, [video?.id, session?.workerUrl]);
    ...
    const handleMessage = (event) => {
      ...
      if (info) {
        ...
        if (stateName && stateName !== lastLoggedState.playerState) {
          ...
          telemetryRef.current?.onStateChange(stateName, streamTime);
        ...
        if (typeof streamTime === 'number') {
          ...
          telemetryRef.current?.onProgress(streamTime, isPlayingRef.current);
  ...
  }, [session, filterSettings, video?.id]);
  ...
  }, [session, filterSettings, prefetchRelated]);
