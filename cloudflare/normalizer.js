...
export function normalizeWatchNextResponse(rawJson) {
  ...
  if (primaryInfo) {
    ...
    const views = extractRunsText(primaryInfo.viewCount?.videoViewCountRenderer?.viewCount);
    const publishedAt = extractRunsText(primaryInfo.dateText);
    ...
    if (secondaryInfo) {
      ...
      if (owner) {
        ...
        subscriberCount = extractRunsText(owner.subscriberCountText);
