...
export async function handleInnerTubeRequest(request, url, env, ctx) {
  ...
  if (clientName === 'TVHTML5') clientNameHeader = '7';
  
  const isTvClient = clientNameHeader === '7';
  const clientVersion = request.headers.get('X-YouTube-Client-Version') || (isTvClient ? '7.20240901.00.00' : '2.20240901.00.00');
  ...
  if (isTvClient) defaultUa = TV_USER_AGENT;
  ...
  forwardHeaders.set('Referer', isTvClient ? 'https://www.youtube.com/tv' : 'https://www.youtube.com');
