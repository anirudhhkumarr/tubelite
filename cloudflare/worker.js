...
import { handleTelemetryRequest } from './telemetry.js';
...
export default {
  async fetch(request, env, ctx) {
    ...
    // 3. Telemetry Gateway
    if (url.pathname.startsWith('/api/telemetry') || url.pathname.startsWith('/api/stats')) {
      return handleTelemetryRequest(request, url, env, ctx);
    }
