...
/// Centralized device screen and Dynamic Island layout metrics.
/// Derives metrics directly from UIKit window safe areas without hardcoded device tables.
public enum TLScreenMetrics {
    public static let islandWidth: CGFloat = 126
    public static let islandHeight: CGFloat = 36.67
    
    /// Top safe area inset resolved dynamically from the active key window.
    ...
    /// Leading safe area inset (notch/island in landscape).
    public static var leadingSafeAreaInset: CGFloat {
        if let scene = UIApplication.shared.connectedScenes.compactMap({ $0 as? UIWindowScene }).first,
           let window = scene.windows.first(where: { $0.isKeyWindow }) ?? scene.windows.first {
    ...
    /// True if the current device/window has an active Dynamic Island.
    public static func hasDynamicIsland(topInset: CGFloat = topSafeAreaInset) -> Bool {
        topInset >= 50
    }
    
    /// Vertical center of the Dynamic Island physical cutout from the display top edge.
    public static func islandCenterY(topInset: CGFloat = topSafeAreaInset) -> CGFloat {
        if topInset >= 65 { return 38.33 }
        if topInset >= 61 { return 32.33 }
        if topInset >= 50 { return 29.5 }
        return max(topInset / 2, 20)
    }
    
    /// Symmetrically balanced top padding around the Dynamic Island:
    /// Distance from display top to island top == Distance from island bottom to content top.
    public static func symmetricTopPadding(topInset: CGFloat = topSafeAreaInset) -> CGFloat {
        if hasDynamicIsland(topInset: topInset) {
            return 2 * islandCenterY(topInset: topInset)
        }
        return max(topInset, 44)
    }
}
...
/// Floating error bubble / toast displaying feedback when network or YouTube API actions fail.
public struct TLErrorBubbleView: View {
    ...
    public var body: some View {
        ...
        .background(
            Capsule()
                .fill(Color(red: 0.14, green: 0.14, blue: 0.16).opacity(0.96))
                .overlay(
                    Capsule()
                        .stroke(TLTheme.danger.opacity(0.45), lineWidth: 1)
                )
